...
class GenerateGaussianNoise():
    def __init__(self, input_shape=(10,), n_params=2, n_summaries=2, n_s=1000,
                 n_d=1000, n_d_small=100, θ_fid=np.array([0., 1.]),
                 δθ=np.array([0.1, 0.1]), training_seed=0, validation_seed=1,
                 additive_noise=None):
        ...
        self.δθ = δθ
        if self.δθ is None:
            self.fiducial_only = True
        else:
            self.half_δθ = δθ / 2.
            self.fiducial_only = False
    ...
    def check_selection(self, size):
        if size not in ["full", "all", "small"]:
            print("size must be `full`, `all` or `small` describing, "
                  "respectively whether just `n_d=n_s` is returned, or "
                  "`n_d=n_s` and `n_d_small` is returned, or `n_d=n_d_small` "
                  "is returned.")
            sys.exit()

    def check_ftype(self, ftype):
        if ftype not in ["both", "numpy", "tfrecords"]:
            print("size must be `both`, `numpy` or `tfrecords` describing, "
                  "respectively whether both `numpy` and `tfrecords` files "
                  "are saved, or just either one.")
            sys.exit()
    ...
    def simulator(self, parameters, seed=None, simulator_args=None):
        ...
        if self.additive_noise is not None:
            simulation += np.random.normal(
                0, self.additive_noise, size=simulation.shape)
        return simulation
    ...
    def generate_data(self, size="full"):
        ...
        if not self.fiducial_only:
            ...
            return (details, a_0, a_1)

    def save(self, ftype="both", size="full", directory="data",
             record_size=0.01):
        ...
        if (ftype=="both") or (ftype=="numpy"):
            ...
            np.save("{}/validation_fiducial.npy".format(directory),
                result[2])
            if not self.fiducial_only:
                np.save("{}/derivative.npy".format(directory), result[3])
                np.save("{}/validation_derivative.npy".format(directory),
                    result[4])
                if size == "all":
                    np.save("{}/derivative_small.npy".format(directory),
                        result[5])
                    np.save("{}/validation_derivative_small.npy".format(
                        directory), result[6])
        ...
        if (ftype=="both") or (ftype=="tfrecords"):
            ...
            if not self.fiducial_only:
                writer.write_record(
                    n_sims=result[0]["n_d"],
                    get_simulation=lambda x, y, z : self.get_derivative(
                        x, y, z, result[3]),
                    fiducial=False,

...
def fft(X, L=None, Lk=None, a=0, b=2 * np.pi, left_edge=None, axes=None, ret_cubegrid=False):
    ...
    dx = np.asarray([float(l) / float(n) for l, n in zip(L, N)])

    freq = np.asarray([fftfreq(n, d=d, b=b) for n, d in zip(N, dx)])
...
def ifft(X, Lk=None, L=None, a=0, b=2 * np.pi, axes=None, left_edge=None, ret_cubegrid=False):
    ...
    N = np.asarray([X.shape[axis] for axis in axes])
    ...
    V = np.product(Lk)
    dk = np.asarray([lk / n for lk, n in zip(Lk, N)], float)

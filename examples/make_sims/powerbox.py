...
class PowerBox(object):
    ...
    def __init__(self, N, pk, seed, dim=2, boxlength=1.0, ensure_physical=False, a=1., b=1.,
    ...
    def gauss_hermitian(self):
        ...
        #if self.seed:
            #jax.random.seed(self.seed)

        mag = jax.random.normal(self.seed, shape=[self.n] * self.dim)
        pha = 2 * np.pi * jax.random.uniform(self.seed, shape=[self.n] * self.dim)
    ...
    def delta_k(self):
        ...
        gh = jax.ops.index_update(gh, jax.ops.index[...], np.sqrt(p) * gh)
        #gh[...] = np.sqrt(p) * gh
        return gh
...
class LogNormalPowerBox(PowerBox):
    ...
    def correlation_array(self):
        ...
        pa = jax.ops.index_update(pa, jax.ops.index[...], self.power_array())
        #pa[...] = self.power_array()
    ...
    def gaussian_power_array(self):
        ...
        gca = jax.ops.index_update(gca, jax.ops.index[...], self.gaussian_correlation_array())
        #gca[...] = self.gaussian_correlation_array()
        gpa = np.abs(dft.fft(gca, L=self.boxlength, a=self.fourier_a, b=self.fourier_b)[0])
        gpa = jax.ops.index_update(gpa, jax.ops.index[self.k() == 0], 0.)
        #gpa[self.k() == 0] = 0
        return gpa
    ...
    def delta_x(self):
        ...
        dk = jax.ops.index_update(dk, jax.ops.index[...], np.sqrt(self.V) * dft.ifft(dk, L=self.boxlength, a=self.fourier_a, b=self.fourier_b)[0])
        dk = np.real(dk)

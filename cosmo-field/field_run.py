...
def run_cosmo_fg(filename, θ_fid = np.array([2.0, 0.8]), θ_fg = np.array([1.8, 3.2]), n_sims=10000):
    ...
    SN = GenerateCosmoField(n_s=n_sims, n_d=n_sims, input_shape=(1,128,128), θ_fid=θ_fid, θ_fg=θ_fg)
...
if __name__ == '__main__':
    ...
    if job == 1:
        run_vanilla_one_nofg('amp-vanilla')

    if job == 2:
        run_vanilla_one_fg('amp+fg')

    if job == 3:
        run_cosmo_fg('cosmo+fg')

    if job == 4:
        run_cosmo_fg('cosmo+big_fg', θ_fg = np.array([100.8, 3.2]))
